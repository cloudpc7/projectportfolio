names =["Angie", "Mary", "Julie", "Hailey", "Missy", "Jules", "Kacy", "Denise", "Jojo"]
const goodWifey = ["Angie", "Julie", "Hailey"];
const maxTries = 3;
let attempts = 0;

function whoIsMyWife(names) {
    var numberOfPeople = names.length;
    var randomWifey = Math.floor(Math.random() + numberOfPeople);
...
function shootYourShot() {
    ...
    if (goodWifey.includes(randomWifey)) {
        ...
        alert(`You are marrying ${randomWifey}. Unfortunately, she only wanted you to elevate her status and your money. She had multiple affairs behind your back, embezzled your money, and even attempted to hire a hitman to kill you for the insurance money. Luckily, you hired a private detective who foiled the assasination attempt by bribing the hitman with a higher bid and turn her in.`)
        if (attempts < maxTries) {
            alert(`You have ${maxTries - attempts} more tries.`);
        } else {
            alert("You died! You were assasinated by her hitman and she ran off with your money with a stud in his early twenties, an up and coming movie star. Better luck next lifetime!");
        }
    }
}
